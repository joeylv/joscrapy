...
ORM，全称为Object/Relation
Mapping，即对象/关系数据库映射，我们可以把他理解成一种规范。它完成面向对象的编程语言到关系数据库的映射。因此，我们可以把ORM框架作为面向对象编程语言到数据库之间的桥梁。
...
先下载一个hibernate的压缩包。将包下的hibernate3.jar和lib路径下的required、jpa子目录下所有JAR包添加到应用程序的WEB-
INF/lib路径下。注意由于hibernate是基于JDBC的，因此还需要添加一个相应数据库的JDBC驱动。
