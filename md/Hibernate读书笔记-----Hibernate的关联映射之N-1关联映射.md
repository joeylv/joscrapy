...
Employee端增加了Department属性，该属性并不是一个普通的组件属性，而是引用另一个持久化类的类。Hibernate使用<many-to-
one.../>元素映射N—1的关联实体，直接采用<many-to-
one.../>元素来映射关联实体将会在N的一端的数据表中增加一个外键，用于参照主表记录。
...
上面的代码就会报空异常：org.hibernate.PropertyValueException: not-null property references
...
使用<join.../>元素映射到连接表时，需要外键关联，应在配置文件中增加<key.../>子元素来映射外键，并为<join.../>元素增加<many-
to-one.../>子元素来映射N-1的关联实体。如下:
