...
def get_urls():
    ...
    """
    page_urls = ['http://www.mmjpg.com/home/{cnt}'.format(cnt=cnt)
                 for cnt in range(2, 3)]
    print("Please wait for second ...")
    page_urls.append('http://www.mmjpg.com')
    img_urls = []
    for page_url in page_urls:
        try:
            cont = requests.get(page_url, headers=HEADERS, timeout=10)
            cont.encoding = "utf-8"
            bs = BeautifulSoup(cont.text, 'lxml').find('div', class_="pic").find('ul')
...
def urls_crawler(url):
    ...
    try:
        ...
        folder_name = BeautifulSoup(r, 'lxml').find(
            'div', class_="content").find('img')['alt'].replace("?", " ")
        with lock:
            if make_dir(folder_name):
                # 套图里图片张数
                max_count = BeautifulSoup(r, 'lxml').find(
                    'div', id='page').find_all('a')[-2].get_text()
                ...
                for _, page_url in enumerate(page_urls):
                    ...
                    img_url = BeautifulSoup(result, 'lxml').find(
                        'div', class_="content").find('a').find('img')['src']
